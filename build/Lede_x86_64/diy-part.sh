...
sed -i "s/OpenWrt /ydfk Compiled in $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ          # 增加个性名字281677160

sed -i "/uci commit system/i\uci set system.@system[0].hostname='OpenWrt-ydfk'" $ZZZ            # 修改主机名称为x86-64-OpenWrt
